...
# Specify the subdirectory path
    ...
    logger.log(logging.INFO, 'registering operations')  
    ...
    logger.log(logging.INFO, 'finished registering operations')
    ...
    if not os.path.exists(configFileLocation):
        logger.log(logging.CRITICAL, "missing \'config.json\' file at " + configFileLocation)
        raise Exception("missing \'config.json\' file at " + configFileLocation)
    
    openeoip_configfile = open(configFileLocation)
    logger = logging.getLogger('openeo')
    logger.log(logging.INFO, 'reading udfs')
    ...
    for udf_location in udf_locations:
        ...
        if not os.path.exists(location):
             logger.log(logging.WARNING, 'udf location doesnt exist: ' + location)
             continue
    ...
    logger.log(logging.INFO, 'finished reading udfs')
    return operationsMetaData

def loadOperationsFolder(folder,operationsMetaData):
    logger = logging.getLogger('openeo')
    if not os.path.exists(folder):
        logger.log(logging.WARNING, 'operation location doesnt exist: ' + folder)
        return operationsMetaData  
